...
from src.gmail.EmailSender import EmailSender
import os


FILEPATH = "/Users/mahcheeteng/Documents/theGivingCook/src/data/theGivingCook.db"

database_loc = f"{FILEPATH}"
...
def insert_report(order, description):
    try:    
        ...
        if (count[0] >= 1):
            send_warning_email(vendor_name, order[1], count[0])
...
def show_report_form_page():
    ...
    if order:
        ...
        item_id, item_name, vendor_id, dates = zip(*order)
    else:
    ...
    with st.form(key='report_form'):
        ...
        if submit_button:
            if not order or not description:
                ...
                insert_report(order[1], description)
                st.session_state.report_order = order[1]
...
def getItemsAndVendors():
    try:
        ...
        # Now use the flattened item_ids list in the next query
        cur.execute("""
            SELECT id, food_name, vendor_id, date_of_entry 
            FROM inventory 
            WHERE id IN ({})
...
def send_warning_email(vendor, time, count):
    ...
    sender = EmailSender()
    sender.email_sender(email, subject, body)    
